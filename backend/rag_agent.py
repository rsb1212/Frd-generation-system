...
#             return analysis
#         except Exception as e:
#             logger.error(f"Failed to understand requirement: {str(e)}")
#             return {"error": str(e)}
...
#     def generate_frd_sections(self, requirement: str, similar_docs: List[Dict]) -> Dict[str, str]:
#         """Generate FRD sections using GPT and context"""
#         try:
#             context = self._build_context(similar_docs)
            
#             prompt = f"""Generate detailed FRD sections based on the requirement and similar documents.

# CURRENT REQUIREMENT:
# {requirement}

# SIMILAR PREVIOUS REQUIREMENTS (for reference):
# {context}

# Generate the following sections in detail:
# 1. Project Overview
# 2. Scope
# 3. Functional Requirements (with details)
...
#             return sections
#         except Exception as e:
#             logger.error(f"Failed to generate FRD sections: {str(e)}")
#             return {"error": str(e)}
...
#     def validate_requirement(self, requirement: str) -> Tuple[bool, List[str]]:
#         """Validate requirement completeness"""
#         issues = []
        
#         # Minimum length check - reduced from 50 to 10 characters
#         if len(requirement.strip()) < 10:
#             issues.append("Requirement is too brief. Please provide more details.")
        
#         # Skip keyword check - any descriptive text is acceptable
#         # keywords = ['should', 'must', 'shall', 'will', 'can', 'feature', 'requirement']
#         # if not any(keyword in requirement.lower() for keyword in keywords):
#         #     issues.append("Requirement lacks clear functional language.")
...
Format as JSON with section names as keys."""
            ...
            logger.exception("Failed to generate FRD sections")
            return {"error": str(e)}
